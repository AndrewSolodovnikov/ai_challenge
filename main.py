...
def create_summary(messages):
    ...
    try:
        summary_prompt = "Please provide a concise summary of the following conversation, capturing the key points, topics discussed, and main conclusions:\n\n"
...
def compact_conversation(messages, compact_threshold):
    ...
    # Return summary + recent messages and flag that compaction occurred
...
def chat():
    try:
        ...
        print(
            f"[LOG] Model: {model}, System: {system_prompt}, Temp: {temperature}, Stats: {show_stats}, Compact: {enable_compact}, Threshold: {compact_threshold}")
        ...
        # Apply compacting if enabled and threshold is reached
        if enable_compact and len(conversations[sid]) > compact_threshold:
            print(f"[LOG] Conversation length: {len(conversations[sid])}, applying compact")
            conversations[sid], was_compacted = compact_conversation(conversations[sid], compact_threshold)
            if was_compacted:
                compact_notification = "🔄 History compressed: Older messages were summarized to save tokens and improve performance."
        ...
        formatted_response = format_response(response, output_format, elapsed_time, input_tokens, output_tokens,
                                             show_stats)
...
if __name__ == '__main__':
    ...
    print("[INFO] Conversation compacting enabled")
